...
function createMQTTClinet(config, app) {
  ...
  if (!config) {
    app.logger.error('[eyu-egg-mqtt]', 'Failed to create MQTT client: config is null.')
    return null
  }
  let workers = process.env.EGG_WORKERS || 1
  config.clientId = `${config.clientIdPrefix}${process.pid % workers + 1}`
  try {
    ...
    client.on('connect', () => {
      app.logger.info(`[eyu-egg-mqtt] MQTT client(${config.clientId}) is connected.`)
    })
